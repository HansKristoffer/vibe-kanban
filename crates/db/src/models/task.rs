...
impl Task {
  ...
  CASE WHEN EXISTS (
    SELECT 1
     ...
     LIMIT 1
  ) THEN 1 ELSE 0 END            AS "has_queued_attempt!: i64",
...
ORDER BY t.created_at DESC"#,
        ...
        let tasks = records
            ...
            .map(|rec| TaskWithAttemptStatus {
                ...
                has_queued_attempt: rec.has_queued_attempt != 0,
    ...
    /// Find active tasks (not Done or Cancelled) for a project with attempt status
    pub async fn find_active_by_project_id_with_attempt_status(
        pool: &SqlitePool,
        project_id: Uuid,
    ) -> Result<Vec<TaskWithAttemptStatus>, sqlx::Error> {
        let records = sqlx::query!(
