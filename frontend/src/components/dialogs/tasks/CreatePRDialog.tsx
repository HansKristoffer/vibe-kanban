...
import { AssetThumbnail } from '@/components/ui-new/primitives/AssetThumbnail';
...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    const { assets, isLoading: assetsLoading } = useWorkspaceAssets(
      attempt?.id,
      { enabled: modal.visible && !!attempt?.id, refetchInterval: false }
    );
            ...
            {!isLoaded ? (
              ...
              <div className="space-y-4 py-4">
                ...
                {!assetsLoading && assets.length > 0 && (
                  <div className="space-y-2">
                    <Label>{t('createPrDialog.assetsLabel', 'Include Assets')}</Label>
                    <p className="text-xs text-muted-foreground">
                      {t('createPrDialog.assetsHint', 'Select screenshots or videos to include in the PR description')}
                    </p>
                    <div className="grid grid-cols-3 gap-2 max-h-32 overflow-y-auto p-1 border rounded">
                      {assets.map((asset) => (
                        <AssetThumbnail
                          key={asset.id}
                          asset={asset}
                    ...
                    {selectedAssetIds.length > 0 && (
                      <p className="text-xs text-muted-foreground">
                        {t('createPrDialog.assetsSelected', '{{count}} asset(s) selected', { count: selectedAssetIds.length })}
                      </p>
                    )}
                  </div>
                )}
